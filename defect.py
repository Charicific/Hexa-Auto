...
async def main(session_file):
    ...
    async def battle(event):
        ...
        if "Wild" in event.raw_text:
                    ...
                    if low_lvl:
                        ...
                        try:
                            ...
                            if pok_name in regular_ball:
                                ...
                                if "☆" in last_two_messages:
                                    await asyncio.sleep(1)
                                    await click_button(event, text="Repeat")
                                elif not "☆" in last_two_messages:
                                    await asyncio.sleep(1)
            ...
            elif any('✨' in message.message.lower() for message in last_two_messages and "☆" in last_two_messages):
                await asyncio.sleep(cooldown)
                try:
                    await click_button(event, text="Poke Balls")
                    await asyncio.sleep(1)
                    ...
                    print(f"Failed to click Poke Balls for {pok_name}")
            elif any('✨' in message.message.lower() for message in last_two_messages and not "☆" in last_two_messages):
                await asyncio.sleep(cooldown)
                try:
                    await click_button(event, text="Poke Balls")
                    await asyncio.sleep(1)
